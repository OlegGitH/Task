...
func TestImportInvalidData(t *testing.T) {
	...
	data, err := importer.ImportDomainData()
	if err != nil {
		t.Error("should handle invalid data gracefully", err)
	}
	// Should still return valid domains, skipping invalid ones
	if len(data) == 0 {
		t.Error("should return valid domains even with some invalid data")
	}
}
